...
Hooks.once("socketlib.ready", () => {
  heraldHud_journalingSocket = socketlib.registerModule("herald-hud");
...
async function heraldHud_getViewQuest() {
  ...
  const typeQuest = [
    {
      name: "Main Quest",
      src: "/modules/herald-hud/assets/quest-icon/main_quest.png",
    },
    {
      name: "Side Quest",
      src: "/modules/herald-hud/assets/quest-icon/side_quest.png",
    },
    {
      name: "Players Quest",
      src: "/modules/herald-hud/assets/quest-icon/player_quest.png",
    },
    {
      name: "Characters Quest",
      src: "/modules/herald-hud/assets/quest-icon/ch_quest.png",
    },
  ];
  let listTypeQuest = "";
      ...
      if (typeContainer) {
        typeContainer.addEventListener("click", () => {
          ...
          const optionsHtml = typeQuest
            .map((q, i) => {
              const checked = q.name === currentType ? "checked" : "";
              return `
          <label style="display:flex; align-items:center; gap:8px; margin-bottom:6px;">
            <input type="radio" name="questType" value="${q.name}" ${checked} />
            <span>${q.name}</span>
          </label>
        `;
            })
          ...
          new Dialog({
            title: "Change Type Quest",
            content: `<form>${optionsHtml}</form>`,
            buttons: {
              ok: {
                label: "Simpan",
                callback: async (html) => {
                  const selected = html
                    .find("input[name='questType']:checked")
                    .val();
                  if (!selected) return;
